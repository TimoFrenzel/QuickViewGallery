...
class PHPPI
{
    ...
    public function showSetup($step)
    {
    ...
    public function showAdmin()
    {
        ...
        } elseif ($_SERVER['QUERY_STRING'] == 'settings') {
            if (isset($_POST['save_settings'])) {
                if ($this->checkErrors()) {
                    ...
                    if (!$this->importSettings()) {
                        $this->showError('Unable to import settings, your old or current settings file may contain errors. Remove the file and try again.');
                        exit;
                    }
    ...
    public function reqCheck()
    {
        ...
        if (is_writeable($this->vars['dir']['local'])) {
            $tests['WRITE_PHPPI_FOLDER'] = [true, '<b>'.$this->vars['dir']['local'].'</b> is writable'];
        } else {
            $tests['WRITE_PHPPI_FOLDER'] = [false, '<b>'.$this->vars['dir']['local'].'</b> is not writable'];
        }
        ...
        if (extension_loaded('gd')) {
            if (function_exists('gd_info')) {
                ...
                $version = $m[0];
                if ($version == 2) {
                    $tests['GD_INSTALLED'] = [true, '<b>GD Version 2.0</b> is installed'];

                    //GD GIF support
                    $gd = gd_info();
                    if ($gd['GIF Read Support'] == true && $gd['GIF Create Support'] == true) {
                        $tests['GD_GIF_SUPPORT'] = [true, 'Your GD install has <b>GIF support</b>'];
                    } else {
                        $tests['GD_GIF_SUPPORT'] = [false, 'Your GD install is missing <b>GIF support</b>'];
                    }

                    //GD JPEG support
                    $gd = gd_info();
                    if ($gd['JPEG Support'] == true) {
                        $tests['GD_JPEG_SUPPORT'] = [true, 'Your GD install has <b>JPEG support</b>'];
                    } else {
                        $tests['GD_JPEG_SUPPORT'] = [false, 'Your GD install is missing <b>JPEG support</b>'];
                    }

                    //GD PNG support
                    $gd = gd_info();
                    if ($gd['PNG Support'] == true) {
                        $tests['GD_PNG_SUPPORT'] = [true, 'Your GD install has <b>PNG support</b>'];
                    } else {
                        $tests['GD_PNG_SUPPORT'] = [false, 'Your GD install is missing <b>PNG support</b>'];
                    }
                } else {
                    $tests['GD_INSTALLED'] = [false, '<b>GD</b> is installed but isn\'t version 2.0'];
                }
            } else {
                $tests['GD_INSTALLED'] = [false, '<b>GD Version 2.0</b> is not installed or your PHP version is less than 4.3.0'];
            }
        } else {
            $tests['GD_INSTALLED'] = [false, '<b>GD Version 2.0</b> is not installed'];
        }
    ...
    {
        if ($_POST['site_name'] == '') {
            $this->vars['errors']['site_name'] = 'Field is empty';
        }
        if ($_POST['theme'] == '') {
            $this->vars['errors']['theme'] = 'No theme selected';
        } else {
            if (!is_dir('../themes/gallery/'.$_POST['theme'])) {
                $this->vars['errors']['theme'] = 'Theme does not exist';
            }
        }
        if ($_POST['thumbnail_theme'] == '') {
            $this->vars['errors']['thumbnail_theme'] = 'No theme selected';
        } else {
            if (!is_dir('../themes/thumbnail/'.$_POST['thumbnail_theme'])) {
                $this->vars['errors']['thumbnail_theme'] = 'Theme does not exist';
            }
        }
        if ($_POST['admin_password'] == '') {
            $this->vars['errors']['admin_password'] = 'Field is empty';
        }
        if ($_POST['thumb_size_small'] == '') {
            $this->vars['errors']['thumb_size_small'] = 'Field is empty';
        }
        if ($_POST['thumb_size_medium'] == '') {
            $this->vars['errors']['thumb_size_medium'] = 'Field is empty';
        }
        if ($_POST['thumb_size_large'] == '') {
            $this->vars['errors']['thumb_size_large'] = 'Field is empty';
        }
        if ($_POST['use_popup_image_viewer'] == 'true' && $_POST['popup_thumb_size'] == '') {
            $this->vars['errors']['popup_thumb_size'] = 'Field is empty';
        }
        if ($_POST['access_log'] == 'on' && $_POST['log_timezone'] == '') {
            $this->vars['errors']['log_timezone'] = 'Field is empty';
        }
        if ($_POST['gzip_compression_level'] == '') {
            $this->vars['errors']['gzip_compression_level'] = 'Field is empty';
        } else {
            if (!$this->isInt($_POST['gzip_compression_level'])) {
                $this->vars['errors']['gzip_compression_level'] = 'Value must be a number';
            }
            if ($_POST['gzip_compression_level'] < 0 || $_POST['gzip_compression_level'] > 9) {
                $this->vars['errors']['gzip_compression_level'] = 'Value must be 0 to 9';
            }
        }
        if ($_POST['jpeg_quality'] == '') {
            $this->vars['errors']['jpeg_quality'] = 'Field is empty';
        } else {
            if (!$this->isInt($_POST['jpeg_quality'])) {
                $this->vars['errors']['jpeg_quality'] = 'Value must be a number';
            }
            if ($_POST['jpeg_quality'] < 0 || $_POST['jpeg_quality'] > 100) {
                $this->vars['errors']['jpeg_quality'] = 'Value must be 0 to 100';
            }
        }
        if ($_POST['gd_cache_expire'] == '') {
            $this->vars['errors']['gd_cache_expire'] = 'Field is empty';
        } else {
            if (!$this->isInt($_POST['gd_cache_expire'])) {
                $this->vars['errors']['gd_cache_expire'] = 'Value must be a number';
            }
        }
        if ($_POST['expire_file_cache'] == '') {
            $this->vars['errors']['expire_file_cache'] = 'Field is empty';
        } else {
            if (!$this->isInt($_POST['expire_file_cache'])) {
                $this->vars['errors']['expire_file_cache'] = 'Value must be a number';
            }
        }
        if ($_POST['thumb_size_large'] == '') {
            $this->vars['errors']['thumb_size_large'] = 'Field is empty';
        ...
        if ($_POST['use_gd'] == 'true') {
            if ($_POST['cache_folder'] == '') {
                $this->vars['errors']['cache_folder'] = 'Field is empty';
            }
        } else {
            if ($_POST['thumbs_folder'] == '') {
                $this->vars['errors']['thumbs_folder'] = 'Field is empty';
            }
        }
        if ($_POST['log_timezone'] == '') {
            $this->vars['errors']['log_timezone'] = 'Field is empty';
        }
        if ($_POST['gallery_folder'] == '') {
            $this->vars['errors']['gallery_folder'] = 'Field is empty';
        } else {
            if (substr($_POST['gallery_folder'], 0, 1) == '/') {
                if (!is_dir($_POST['gallery_folder'])) {
                    $this->vars['errors']['gallery_folder'] = 'Folder does not exist';
                }
            } else {
                if (!is_dir('../../'.$_POST['gallery_folder'])) {
                    $this->vars['errors']['gallery_folder'] = 'Folder does not exist';
    ...
    public function loadSettingsFields()
    {
        $this->vars['settings'] = [
            'General' => [
                'Site Name' => [
            ...
            'Thumbnails' => [
                ...
                'Use Cache for Folder Thumbnails' => [
    ...
    public function populateThemes()
    {
        ...
        if (is_dir($gallery_dir)) {
            if ($dh = opendir($gallery_dir)) {
                ...
                $this->vars['settings']['General']['Theme']['data'] = ['No themes found' => ''];
            }
        } else {
            $this->vars['settings']['General']['Theme']['data'] = ['No themes found' => ''];
        }
        ...
        if (is_dir($thumb_dir)) {
            if ($dh = opendir($thumb_dir)) {
                ...
                $this->vars['settings']['General']['Thumbnail Theme']['data'] = ['No themes found' => ''];
            }
        } else {
            $this->vars['settings']['General']['Thumbnail Theme']['data'] = ['No themes found' => ''];
        }
    ...
    public function outputSettingsFields()
    {
    ...
    public function initialize()
    {
        ...
        } elseif (!is_file('../settings.php') && !is_file('../phppi_settings.php')) {
        ...
        if (!isset($_SESSION['password']) && ($this->vars['setup_mode'] == 'admin' || $this->vars['setup_mode'] == 'upgrade')) {
            ...
            $this->showLogin();
        } elseif (isset($_SESSION['password']) && $this->vars['setup_mode'] == 'admin') {
